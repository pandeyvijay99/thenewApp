...
const thoughtssubcommentModel = require("../models/thoughtssubcomment")
...
const postComment = async (req, res) => {
  ...
  try {
    ...
    const commentData = {
      user_id: user_id,
      thoughts_id: req.body.thoughts_id,
      comment: req.body.comment
    }
    debugger;
    const comment = await ThoughtsComment.create(commentData);
    if (comment) {
      await Thoughts.findByIdAndUpdate(req.body.thoughts_id, {
...
const postSubComment = async (req, res) => {
  ...
  try {
    ...
    thoughtssubcommentModel.create(commentData).then((data, err) => {
...
const fetchComment = async (req, res) => {
  try {
    ...
    const result = await ThoughtsComment.aggregate([
      {
        $match: { thoughts_id }, // Match comments by the given photo_id
      },
      {
        $lookup: {
      ...
      {
        $lookup: {
          from: "thoughtssubcomments", // Join with subcomments collection
...
const fetchSubComment = async (req, res) => {
  ...
  try {
    ...
    const result = await thoughtssubcommentModel.aggregate([
      {
        $match: {
...
const postCommentReaction = async (req, res) => {
  ...
  try {
    ...
    console.log("filer is ", filter);
    const result = await ThoughtsComment.findOneAndUpdate(filter, { $push: { commentReaction: commentReaction } }, {
      returnOriginal: false
    });
    return res.status(StatusCodes.OK).json({
      statusCode: 0,
...
const fetchCommentReaction = async (req, res) => {
  ...
  try {
    ...
    result = await ThoughtsComment.aggregate([
      {
    ...
    grp = await ThoughtsComment.aggregate([
      {
        $match: { _id: new ObjectId(req.body.comment_id) } // Match the parent document with the given ID
      },
      {
