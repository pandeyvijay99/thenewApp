...
const postVideoComment = async (req, res) => {
  ...
  try {
    ...
    debugger;
    const comment = await VideoComment.create(videocommentData)
    // .then((data, err) => {
    //   if (err) 
    // });
    if (comment) {
      ...
      sendPushNotification(video.video_user_id, user_id, "videoComment", req.body.video_id);
...
const fetchVideoComment = async (req, res) => {
  try {
    ...
    const result = await VideoComment.aggregate([
      {
        $match: { video_id }, // Match comments by the given video_id
      },
      {
        $lookup: {
      ...
      {
        $lookup: {
          from: "videosubcomments", // Join with subcomments collection
