...
const podcastsubcommentModel = require("../models/podcastsubcomment")
...
const postPodcastComment = async (req, res) => {
  ...
  try {
    ...
    const podcastcommentData = {
      user_id: user_id,
      podcast_id: req.body.podcast_id,
      comment: req.body.comment
    }
    debugger;
    const comment = await PodcastComment.create(podcastcommentData);

    if (comment) {
      await Podcast.findByIdAndUpdate(req.body.podcast_id, {
        $inc: { commentCount: 1 }, // Increment commentCount by 1
      });
      console.log('comment data', podcastcommentData);
      const podcast = await Podcast.findById(req.body.podcast_id)
      sendPushNotification(podcast.podcast_user_id, user_id, "podcastComment", req.body.podcast_id);
...
const postPodcastSubComment = async (req, res) => {
  // console.log("validation ")
  try {
    debugger;
    console.log("inside validation ")
    ...
    podcastsubcommentModel.create(podcastcommentData).then((data, err) => {
...
const fetchPodcastComment = async (req, res) => {
  ...
  try {
    ...
    const result = await PodcastComment.aggregate([
      {
        $match: { podcast_id }, // Match comments by the given podcast_id
      },
      {
        $lookup: {
      ...
      {
        $lookup: {
          from: "podcastsubcomments", // Join with subcomments collection
...
const fetchPodcastSubComment = async (req, res) => {
  ...
  try {
    ...
    const result = await podcastsubcommentModel.aggregate([
      {
        $match: {
...
const postPodcastCommentReaction = async (req, res) => {
  ...
  try {
    ...
    console.log("filer is ", filter);
    const result = await Comment.findOneAndUpdate(filter, { $push: { commentReaction: podcastcommentReaction } }, {
      returnOriginal: false
    });
    return res.status(StatusCodes.OK).json({
      statusCode: 0,
...
const fetchPodcastCommentReaction = async (req, res) => {
  ...
  try {
    ...
    result = await PodcastComment.aggregate([
      {
    ...
    grp = await PodcastComment.aggregate([
      {
        $match: { _id: new ObjectId(req.body.comment_id) } // Match the parent document with the given ID
      },
      {
...
module.exports = { postPodcastComment, postPodcastSubComment, fetchPodcastComment, fetchPodcastSubComment, postPodcastCommentReaction, fetchPodcastCommentReaction };