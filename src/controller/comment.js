...
const postComment = async (req, res) => {
  ...
  try {
    ...
    if (comment) {
      await Blip.findByIdAndUpdate(req.body.blip_id, {
        $inc: { commentCount: 1 }, // Increment commentCount by 1
      });
      console.log('comment data', commentData);
      const blip = await Blip.findById(req.body.blip_id);
      sendPushNotification(blip.blip_user_id, user_id, "blipComment", req.body.blip_id);
...
const fetchComment = async (req, res) => {
  ...
  try {
    ...
    const result = await Comment.aggregate([
      {
        $match: { blip_id }, // Match comments by the given blip_id
      },
      {
        $lookup: {
      ...
      {
        $lookup: {
          from: "subcomments", // Join with subcomments collection
