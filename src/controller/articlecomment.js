...
const articlesubcommentModel = require("../models/articlesubcomment")
...
const postComment = async (req, res) => {
  ...
  try {
    ...
    debugger;
    const comment = await ArticleComment.create(commentData);
    if (comment) {
...
const postSubComment = async (req, res) => {
  ...
  try {
    ...
    articlesubcommentModel.create(commentData).then((data, err) => {
...
const fetchComment = async (req, res) => {
  try {
    ...
    const result = await ArticleComment.aggregate([
      {
        $match: { article_id }, // Match comments by the given article_id
      },
      {
        $lookup: {
      ...
      {
        $lookup: {
          from: "articlesubcomments", // Join with subcomments collection
...
const fetchSubComment = async (req, res) => {
  ...
  try {
    ...
    const result = await articlesubcommentModel.aggregate([
      {
        $match: {
...
const postCommentReaction = async (req, res) => {
  ...
  try {
    ...
    console.log("filer is ", filter);
    const result = await ArticleComment.findOneAndUpdate(filter, { $push: { commentReaction: commentReaction } }, {
      returnOriginal: false
    });
    return res.status(StatusCodes.OK).json({
      statusCode: 0,
...
const fetchCommentReaction = async (req, res) => {
  ...
  try {
    ...
    result = await ArticleComment.aggregate([
      {
    ...
    grp = await ArticleComment.aggregate([
      {
        $match: { _id: new ObjectId(req.body.comment_id) } // Match the parent document with the given ID
      },
      {
